...
@section('content')
                        ...
                        <div class="card-body pt-3">
                            ...
                            @if (!$isChoice)
                                ...
                                <div class="form-floating form-floating-outline" style="max-width:560px">
                                    <input type="text" class="form-control" name="answer[{{ $q->id }}]" placeholder="Jawaban singkat" {{ $q->is_required ? 'required' : '' }}>
                                    ...
                                    @forelse($opts as $j => $opt)
                                        @if($q->type === 'checkbox')
                                            <div class="form-check mb-2">
                                                <input class="form-check-input" type="checkbox" name="answer[{{ $q->id }}][]" value="{{ $opt->id }}" id="option_{{ $q->id }}_{{ $opt->id }}">
                                                <label class="form-check-label" for="option_{{ $q->id }}_{{ $opt->id }}">
                                                    {{ $opt->answer }}
                                                </label>
                                            </div>
                                        @else
                                            <div class="form-check mb-2">
                                                <input class="form-check-input" type="radio" name="answer[{{ $q->id }}]" value="{{ $opt->id }}" id="option_{{ $q->id }}_{{ $opt->id }}" {{ $q->is_required ? 'required' : '' }}>
                                                <label class="form-check-label" for="option_{{ $q->id }}_{{ $opt->id }}">
                                                    {{ $opt->answer }}
                                                </label>
                                            </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="text-center mb-3">
                        ...
                        <h6 class="mb-2">Apakah Anda yakin ingin mengumpulkan jawaban?</h6>
    ...
    <script>
        function showNotification(message, type = 'success') {
            ...
            // Set icon, title, and style based on type
            if (type === 'success') {
        ...
        // Real-time validation - hilangkan error saat user mulai input
        document.addEventListener('DOMContentLoaded', function() {
            // Untuk text input - hilangkan error saat user mulai mengetik
            ...
            // Untuk radio - hilangkan error saat user memilih option
            document.querySelectorAll('input[type="radio"]').forEach(function(radio) {
                radio.addEventListener('change', function() {
                    const name = this.name;
                    // Hilangkan error dari semua radio dengan name yang sama
            ...
            // Untuk checkbox - hilangkan error saat user memilih option
            document.querySelectorAll('input[type="checkbox"]').forEach(function(checkbox) {
                checkbox.addEventListener('change', function() {
                    const name = this.name;
                    // Hilangkan error dari semua checkbox dengan name yang sama
            ...
            // Handle konfirmasi submit modal
            document.getElementById('confirmSubmitBtn').addEventListener('click', function() {
                ...
                btn.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>Mengumpulkan...';

                // Submit form ke server
                setTimeout(() => {
                    // Submit form biasa tanpa AJAX
        ...
        function submitForm() {
            ...
            // Cek semua input text yang required
            ...
            document.querySelectorAll('input[type="radio"][required]').forEach(function(radio) {
                ...
                if (!isChecked) {
                    // Tandai semua radio dengan name yang sama
                    document.querySelectorAll(`input[name="${name}"]`).forEach(function(r) {
            ...
            if (hasError) {
                ...
                // Semua sudah terisi, tampilkan modal konfirmasi
