...
<body>
    ...
    <main style="margin: 0px 10px 0px 10px;">
        @foreach ($reports as $report)
            ...
            {{-- Logika paginasi per dosen (jika mata kuliahnya banyak) tetap sama --}}
            @foreach ($pagedCourses as $coursesOnPage)
                ...
                <table width="100%">
                    <thead>
                        <tr>
                            ...
                            <th rowspan="2" width="35%" style="vertical-align: middle; text-align:center;">
                                Pertanyaan</th>
                            <th colspan="{{ $coursesOnPage->count() }}"
                                style="vertical-align: middle; text-align:center;">Matakuliah</th>
                        </tr>
                        <tr>
                ...
                {{-- Bagian Keterangan dan TTD hanya akan muncul di halaman terakhir laporan per dosen --}}
                @if ($loop->last)
                    <div style="width: 100%; height:30px; margin-top:10px;">
                        ...
                        <div style="float: left; width: 20%; font-size: 12px;line-height: 15px;">
                            <span>: <strong>{{ number_format($report->overall_average_score, 2) }}</strong></span><br>
                ...
                {{-- Jika mata kuliah dosen ini lebih dari 1 halaman, beri page break --}}
                @if (!$loop->last)
                    <div class="page-break"></div>
                @endif
            @endforeach

            {{-- BAGIAN TERPENTING: Beri page break jika ini BUKAN laporan dosen yang terakhir --}}
            @if (!$loop->last)
                <div class="page-break"></div>
            @endif
        @endforeach
